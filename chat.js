...
// Array of API Keys
const API_KEYS = [
    'AIzaSyB48y2OYiA15OX1AlQGhmx8FxoN-Om7t4E',  
...
async function fetchGeminiResponse(prompt) {
    ...
    for (let i = 0; i < API_KEYS.length; i++) {
        try {
            const response = await fetch(API_URL_BASE + API_KEYS[i], {
                method: 'POST',
            ...
            if (response.ok && data.candidates?.[0]?.content?.parts?.[0]?.text) {
                ...
                console.warn(`API Key ${i + 1} failed:`, data.error?.message || "Unknown error");
            }
        } catch (error) {
    ...
    return "All API keys failed. Please check your API keys or quota limits.";
}
