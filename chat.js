function toggleChat() {
 ...
 function speakText(text) {
    ...
    let lang = /[\u0900-\u097F]/.test(text) ? "hi-IN" : "en-IN"; // Detect Hindi or default to English
    let speechChunks = text.match(/.{1,100}/g) || [text]; // Split Hindi text into smaller chunks
    
    console.log("🎙️ Speaking:", text, "in", lang);

    let voices = speechSynthesis.getVoices();
    let selectedVoice = voices.find(voice => voice.lang.includes(lang)) || voices[0];

    function speakChunk(index) {
        if (index >= speechChunks.length) return; // Stop if all chunks are spoken
        
        let speech = new SpeechSynthesisUtterance(speechChunks[index]);
        speech.lang = lang;
        speech.voice = selectedVoice;
        speech.rate = 1;
        speech.pitch = 1;
        
        console.log("🎤 Using voice:", speech.voice.name);
        
        speech.onend = () => setTimeout(() => speakChunk(index + 1), 50); // Minimize gap between chunks
        speech.onerror = (e) => {
            console.error("⚠️ Speech error:", e);
            setTimeout(() => speakChunk(index), 100); // Retry if error occurs
        };
    ...
    function speakNow() {
        if (voices.length > 0) {
            ...
            speechSynthesis.onvoiceschanged = () => {
                ...
                selectedVoice = voices.find(voice => voice.lang.includes(lang)) || voices[0];
                speakChunk(0);
            };
        }
    }
...
// 🎵 Attach TTS to Sound Icon Click
