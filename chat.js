function toggleChat() {
 ...
 function speakText(text, lang = "auto", rate = 1, pitch = 1, useGoogleVoice = true) {
    if (!text) {
    ...
    if (lang === "auto") {
        lang = /[\u0900-\u097F]/.test(text) ? "hi-IN" : "en-IN"; // Auto-detect Hindi or English
    }
    ...
    let maxChunkSize = 200; // Max speech chunk size
    let speechChunks = [];
    let currentChunk = "";

    // Improved Smart Text Splitting (Respects spaces & punctuation)
    ...
    if (currentChunk) speechChunks.push(currentChunk.trim());
    ...
    function loadVoicesAndSpeak(retries = 5) {
        ...
        // Select Google Hindi Voice for Hindi, English Voice otherwise
        let preferredVoice;

        if (useGoogleVoice) {
            preferredVoice = voices.find(v => v.lang === lang && v.name.includes("Google")) ||
        ...
        if (!preferredVoice) {
            console.warn(`⚠️ No suitable voice found for ${lang}, using default.`);
        }

        function speakChunk() {
            if (speechIndex >= speechChunks.length) return; // Stop after last chunk
            ...
            speech.onend = function () {
                ...
                if (speechIndex < speechChunks.length) {
                    setTimeout(speakChunk, 1); // **ZERO Delay** for natural speech flow
                }
            };
...
// Event Listener for Click on Sound Icons (For Mobile Compatibility)
