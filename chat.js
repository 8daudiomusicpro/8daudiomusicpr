function toggleChat() {
 ...
 // 🗣️ Fixed TTS with Unlimited Looping & Smooth Voice
function speakText(text, lang = "auto", rate = 1, pitch = 1, loop = false) {
    if (!text) {
    ...
    // Remove unwanted special symbols
    ...
    // Split text into 200-character chunks (Speech Synthesis limit)
    ...
    function speakNow() {
        ...
        function speakChunk() {
            if (speechIndex >= speechChunks.length) {
                if (loop) {
                    speechIndex = 0; // Restart speech if looping is enabled
                } else {
                    return; // Stop when all chunks are spoken
                }
            }
            ...
            speech.onend = function () {
                speechIndex++;
                setTimeout(speakChunk, 10); // Ensures smooth speech transition
            };
...
// 🎵 Attach TTS to Sound Icon Click (With Unlimited Looping)
document.addEventListener("click", function (event) {
    if (event.target.classList.contains("sound-icon")) {
        ...
        let loop = event.target.dataset.loop === "true"; // Looping mode

        speakText(text, lang, rate, pitch, loop);
    }
});

// 🔥 Example Usage (Unlimited Loop Enabled)
speakText("", "en-IN", 1.2, 1, true);
speakText("", "hi-IN", 1, 1.1, true);
